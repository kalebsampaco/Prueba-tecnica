...
const DialogEditorCambioSolicitante = (props) => {
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateActoAdmin(body, routeParams.id));
    if (resultBorrador) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getActoAdmin(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createActoAdminReview(body));
    if (result) {
