...
import {
  createAdmisionRecJur,
  updateBorradorAdmisionRec,
  getAdmisionRec,
} from '../../app/main/apps/RolFuncionario/solicitudes/store/AutoAdmiRecSliceDos';
...
const DialogEditorAutRec = (props) => {
  const {
    open,
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorAdmisionRec(body, routeParams.id));
    if (resultBorrador.status === 200) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getAdmisionRec(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createAdmisionRecJur(body));
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewDocumentResolucion(body, 'autoAdmisionRecurso'));
...
export default DialogEditorAutRec;
