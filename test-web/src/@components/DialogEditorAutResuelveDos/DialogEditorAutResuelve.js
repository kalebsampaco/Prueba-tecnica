...
import {
  createResuelveRecRev,
  getResuelveRec,
  updateBorradorResuelveRec,
} from 'app/main/apps/RolFuncionario/solicitudes/store/autoResuelveRecSliceDos';
...
const DialogEditorAutResuelve = (props) => {
  const {
    open,
  ...
  const [editorAutoRecTwo, setEditorAutoRecTwo] = useState(EditorState.createEmpty());
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorResuelveRec(routeParams.id, body));
    if (resultBorrador.status === 200) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getResuelveRec(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createResuelveRecRev(body));
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewDocumentResolucion(body, 'autoResuelveRecurso'));
...
export default DialogEditorAutResuelve;
