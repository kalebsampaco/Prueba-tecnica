...
import {
  updateActoAdminTransladoTerritorialDestino,
  getActoAdminTransladoTerritorialDestino,
  getPreviewActoAdminTransladoTerritorialDestino,
  createActoAdminTransladoTerritorialReviewDestino
} from '../../app/main/apps/RolFuncionario/solicitudes/store/autoTransladoTerritorialSlice';
...
const DialogEditorTransladoTerritorialDestino = (props) => {
  const {
    open,
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateActoAdminTransladoTerritorialDestino(body, routeParams.id));
    if (resultBorrador) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getActoAdminTransladoTerritorialDestino(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createActoAdminTransladoTerritorialReviewDestino(body));
    if (result) {
        ...
        >
          <div className="mb-16 flex justify-between">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Fuente:</span> {fuente}
              </Typography>
            </div>
            {fixes && (
              <div
...
export default DialogEditorTransladoTerritorialDestino;
