...
import {
  createAutoRequerimientoRevision,
  getAutoRequerimiento,
  updateBorradorRequerimiento,
} from 'app/main/apps/RolFuncionario/solicitudes/store/RequerimientoSlice';
...
const DialogEditorRequer = (props) => {
  const {
    open,
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorRequerimiento(body, routeParams.id));
    if (resultBorrador.status === 200) {
      ...
      await dispatch(getAutoRequerimiento(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const resultCreate = await dispatch(createAutoRequerimientoRevision(body));
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewDocumentResolucion(body, 'autoRequerimiento'));
        ...
        >
          <div className="mb-16 flex justify-between">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Fuente:</span> Auto requerimiento
              </Typography>
            </div>
            {fixes && (
              <div
...
export default DialogEditorRequer;
