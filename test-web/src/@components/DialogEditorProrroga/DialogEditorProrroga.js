...
import {
  createAdmisionAutoProrrogaRevision,
  getActoAdminProrroga,
  getPreviewDocumentProrroga,
  handleProrroga,
  updateBorradorAutoInicioProrroga,
} from 'app/main/apps/RolFuncionario/solicitudes/store/ProrrogaSlice';
import correctSignature from 'app/utils/functions/correctSignature';
import ViewProrroga from './components/ViewProrroga';
...
const DialogEditorProrroga = (props) => {
  const {
    open,
  ...
  const [diasProrroga, setDiasProrroga] = useState(null);

  useEffect(() => {
    setDiasProrroga(dataProrroga?.dias_prorroga);
  }, []);
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorAutoInicioProrroga(body, routeParams.id));
    if (resultBorrador.status === 200) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getActoAdminProrroga(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createAdmisionAutoProrrogaRevision(body));
    if (result?.status === 200) {
      await dispatch(
        showMessage({
          message: '¡Mensaje de revisión auto inicio prórroga enviado con exito!',
          variant: 'success',
        })
      );
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewDocumentProrroga(body));
  ...
  const handleAceptarProrroga = async () => {
    const body = {
      aprobado: 2, // Aprobado es 2
      idSol: routeParams?.id,
      dias_prorroga: diasProrroga,
      idProrroga: dataProrroga?.id,
    };

    await dispatch(handleProrroga(body));
  };

  const handleDenegarProrroga = async () => {
    const body = {
      aprobado: 1, // No aprobado es 1
      idSol: routeParams?.id,
      dias_prorroga: diasProrroga,
      idProrroga: dataProrroga?.id,
    };
  ...
  const handleChangeDias = (e) => {
    ...
    const numberProrroga = Number(e.target.value) || e.target.value;
    if (reg.test(numberProrroga) && numberProrroga <= 60) {
      setDiasProrroga(e.target.value);
    } else if (numberProrroga === '') {
      setDiasProrroga('');
    }
  };
        ...
        >
          <div className="flex justify-between mb-16">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Referencia:</span> Auto de solicitud de prórroga
              </Typography>
              <Typography className="text-primary">
          ...
          <div className="grid w-full h-full mb-32" style={{ gridTemplateColumns: '85% 15%' }}>
            ...
            <div className="pl-10">
              <ViewProrroga
                ...
                body={dataProrroga?.justificacion_prorroga}
                dataProrroga={dataProrroga}
              />
              <ViewProrroga
                title="Fecha Solicitud"
                body={dataProrroga?.fecha_creacion}
                dataProrroga={dataProrroga}
              />
              <ViewProrroga
                title="Dias calendario de prórroga solicitados"
                body={dataProrroga?.dias_prorroga}
                dataProrroga={dataProrroga}
                value={diasProrroga}
              ...
              />
              <div
                className="flex flex-col justify-between w-full mt-24"
                style={{ height: '102px' }}
              >
                <CustomButton
                  height="medium"
                  label="Aceptar prórroga"
                  className="outlineSecondaryLight"
                  onClick={handleAceptarProrroga}
                  width="full"
                />
                <CustomButton
                  height="medium"
                  label="Denegar prórroga"
                  className="outlineSecondaryLight"
                  onClick={handleDenegarProrroga}
                  width="full"
                />
              </div>
            </div>
              ...
              <div className="flex">
                ...
                <div>
                  <CustomButton
                    ...
                    className="primary"
                    onClick={handleSure}
                    disabled={dataProrroga?.aprobado === 0}
                  />
                </div>
              </div>
            </div>
...
export default DialogEditorProrroga;
