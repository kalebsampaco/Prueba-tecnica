...
import {
  updateActoAdminCambioSol,
  getActoAdminCambioSol,
  getPreviewCambioSol,
  createActoAdminCambioSolReview,
  updateCambioSolicitante,
} from '../../app/main/apps/solicitudes/store/cambioSolicitanteSlice';
...
const DialogEditorCambioSolicitante = (props) => {
  const {
    open,
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateActoAdminCambioSol(body, routeParams.id));
    if (resultBorrador) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getActoAdminCambioSol(routeParams.id));
  ...
  const handleAceptar = (aceptado) => async () => {
    await dispatch(updateCambioSolicitante(aceptado, routeParams.id));
    await dispatch(
      showMessage({
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createActoAdminCambioSolReview(body));
    if (result) {
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewCambioSol(body));
...
export default DialogEditorCambioSolicitante;
