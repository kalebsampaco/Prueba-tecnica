...
} from 'app/main/apps/RolFuncionario/solicitudes/store/desistimientoTacitoSlice';
...
const DialogEditorDesTac = (props) => {
  ...
  const handleBorrador = async () => {
    ...
    if (resultBorrador.status === 200) {
      ...
      await dispatch(showMessage({ message: resultBorrador.data.message, variant: 'error' }));
    }
  };
  ...
  const hanleRevision = async () => {
    ...
    if (resultCreate.status === 200) {
      await dispatch(showMessage({ message: resultCreate.data.message, variant: 'success' }));
      setDialogEditor(false);
      setModalAlert(false);
      history.push('/solicitudes');
    } else {
      await dispatch(showMessage({ message: resultCreate.data.message, variant: 'error' }));
    }
  };
