...
import { getPreviewResolucion } from 'app/main/apps/RolFuncionario/solicitudes/store/evaluacionesSlice';
...
const DialogRevisionDesTac = (props) => {
  const {
    open,
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewResolucion(body, 'autoDesistimiento'));
  ...
  const itemCorrecionesNames =
    itemCorreciones?.acto_correccion?.length > 0 ? itemCorreciones?.acto_correccion : [];
              ...
              <div className="mb-32">
                <Typography style={{ color: '#FF4D4D' }} className="mb-8">
                  {itemCorreciones?.aa_fecha_creacion
                    ...
                    : ''}
                  - Autor:{' '}
                  {itemCorrecionesNames.length > 0 &&
                    `${itemCorrecionesNames[0]?.fk_creador?.cli_usuario?.cu_nombres}
                      ${itemCorrecionesNames[0]?.fk_creador?.cli_usuario?.cu_apellidos}:`}
                </Typography>
                <div
        ...
        >
              ...
              >
                ...
                <div>
                  ...
                  <Typography className="font-semibold" style={{ color: '#364A5D' }}>
                    Por favor revisa el documento con las siguientes <strong>correcciones</strong>{' '}
                    solicitadas por{' '}
                    {itemCorrecionesNames.length > 0 &&
                      `${itemCorrecionesNames[0]?.fk_creador?.cli_usuario?.cu_nombres}
                      ${itemCorrecionesNames[0]?.fk_creador?.cli_usuario?.cu_apellidos}:`}
          ...
          {status === 4 && disableCode && (
            <CoomonFirmaAdministrativo
              ...
              titleFirma="Firma de Desistimiento tácito"
            />
          )}
            ...
            <div className="flex">
              <div className="mr-10">
                <CustomButton
                  ...
                  className="error"
                  onClick={status === 4 ? onClickDevolverCorreccion : onClickDevolver}
                  disabled={arrayFix?.length === 0 && itemSolicitud?.ss_paso_actual !== 4}
                />
              </div>
              <div className="mr-12">
                <CustomButton
...
export default DialogRevisionDesTac;
