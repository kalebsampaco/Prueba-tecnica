...
import {
  createAutoDesistimientoRevision,
  getAutoDesistimientoExpreso,
  updateBorradorDesistimiento,
} from 'app/main/apps/RolFuncionario/solicitudes/store/desistimientoExpresoSlice';
...
const DialogEditorDesistimiento = (props) => {
  const {
    open,
  ...
  const [editorAutoRequerimiento, setEditorAutoRequerimiento] = useState(EditorState.createEmpty());

  const [editorAutoRequerimientoTwo, setEditorAutoRequerimientoTwo] = useState();

  const rawContentState = convertToRaw(editorAutoRequerimiento.getCurrentContent());
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorDesistimiento(body, routeParams.id));
    if (resultBorrador.status) {
      await dispatch(
        showMessage({
      ...
      await dispatch(getAutoDesistimientoExpreso(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const resultCreate = await dispatch(createAutoDesistimientoRevision(body));
    if (resultCreate.status) {
      await dispatch(
        showMessage({
          message: '¡Solicitud gestionada con éxito!',
          variant: resultCreate.data.status === true ? 'success' : 'error',
        })
      );
      setDialogEditor(false);
      setModalAlert(false);
        ...
        >
          <div className="mb-16 flex justify-between">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Fuente:</span> Auto desistimiento expreso
              </Typography>
            </div>
            {fixes && (
              <div
          ...
          <div className="w-full mb-32 h-full">
            <EditorComponent
              onChange={onChange}
              editorState={editorAutoRequerimiento}
              setEditorState={setEditorAutoRequerimiento}
...
export default DialogEditorDesistimiento;
