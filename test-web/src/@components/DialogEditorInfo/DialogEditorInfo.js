...
import {
  createAutoInfoCompRevision,
  getAutoInfoComp,
  updateBorradorInfoComp,
} from 'app/main/apps/RolFuncionario/solicitudes/store/InfoCompSlice';
...
const DialogEditorInfo = (props) => {
  const {
    ...
    label,
    // label: recibe titulo (Auto requerimiento(xxx), Auto requerimiento...)
    addressee,
    setDialogEditor,
    itemActoAdm,
    itemCorreciones,
  ...
  const [editorAutoInfo, setEditorAutoInfo] = useState(EditorState.createEmpty());
  const [editorAutoInfoTwo, setEditorAutoInfoTwo] = useState();
  const rawContentState = convertToRaw(editorAutoInfo.getCurrentContent());
  const markup = draftToHtml(rawContentState);

  const handleBorrador = async () => {
    const body = {
      aa_estado: 1,
      aa_cuerpo_doc: correctSignature(editorAutoInfoTwo || itemActoAdm?.aa_cuerpo_doc),
    };
    const resultBorrador = await dispatch(updateBorradorInfoComp(body, routeParams.id));
    if (resultBorrador.status === 200) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getAutoInfoComp(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const resultCreate = await dispatch(createAutoInfoCompRevision(body));
  ...
  const hash = {};
  const filteredAutoresCorrecion = itemCorreciones?.acto_correccion
        ...
        >
          <div className="mb-16 flex justify-between">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Fuente:</span> Auto de información complementaria.
              </Typography>
            </div>
            {fixes && (
              <div
          ...
          <div className="w-full mb-32 h-full">
            <EditorComponent
              onChange={onChange}
              editorState={editorAutoInfo}
              setEditorState={setEditorAutoInfo}
              setEditorState2={setEditorAutoInfoTwo}
              editorState2={editorAutoInfoTwo}
