...
import {
  updateActoAdminTransladoTerritorial,
  getActoAdminTransladoTerritorial,
  getPreviewActoAdminTransladoTerritorial,
  createActoAdminTransladoTerritorialReview
...
const DialogEditorTransladoTerritorial = (props) => {
  const {
    open,
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateActoAdminTransladoTerritorial(body, routeParams.id));
    if (resultBorrador) {
      await dispatch(showMessage({ message: '¡Borrador guardado con éxito!', variant: 'success' }));
      await dispatch(getActoAdminTransladoTerritorial(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const result = await dispatch(createActoAdminTransladoTerritorialReview(body));
    if (result) {
...
export default DialogEditorTransladoTerritorial;
