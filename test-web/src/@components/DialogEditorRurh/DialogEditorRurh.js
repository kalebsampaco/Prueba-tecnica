...
const DialogEditorRurh = (props) => {
  ...
  const handleBorrador = async () => {
    ...
    const resultBorrador = await dispatch(updateBorradorAutoInicioRurh(body, routeParams.id));
    if (resultBorrador.status === 200) {
      ...
      await dispatch(getConceptoRurh(routeParams.id));
  ...
  const hanleRevision = async () => {
    ...
    const resultCreate = await dispatch(createDocumentoRevisionRurh(body));
  ...
  const handleViewDocument = async () => {
    ...
    const response = await dispatch(getPreviewDocumentResolucion(body, 'autoRecursoHidrico'));
  ...
  const isJefeWithAdditionalRole =
    itemActoAdm?.fk_firmante?.roles_add?.find(
      (role) => role.fk_rol?.rol_cliente.rc_id_rol === 26 && role.estado === 1
    )?.fk_rol?.rol_cliente?.rc_nombre || 'Jefe Territorial';
  const roleFirmante =
    itemActoAdm?.fk_firmante?.fk_roles?.rol_id !== 26
      ? isJefeWithAdditionalRole
      : itemActoAdm?.fk_firmante?.fk_roles?.rol_cliente?.rc_nombre;
  const rolRurh = itemActoAdm?.fk_firmante?.fk_roles?.rol_cliente?.rc_nombre;
        ...
        >
          <div className="mb-16 flex justify-between">
            <div>
              ...
              <Typography className="text-primary">
                <span className="font-bold">Referencia:</span> Informe técnico
              </Typography>
              <Typography className="text-primary">
          ...
          <div className="flex justify-between w-full" style={{ alignSelf: 'end' }}>
            <div className="flex">
              <div className="mr-32">
                ...
                <Typography className="text-primary font-bold">
                  (
                  {itemGestion.ss_paso_actual === 0 &&
                  tramiteId === 26 &&
                  itemGestion?.ss_id_estado === 29
                    ? rolRurh
                    : roleFirmante}
                  )
                </Typography>
...
export default DialogEditorRurh;
